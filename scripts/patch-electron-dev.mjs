// Patches the Electron.app bundle in node_modules to show "ZAI Agent" name and icon in macOS dock during dev mode.
...
if (existsSync(plistPath)) {
  try {
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName ZAI Agent" "${plistPath}"`)
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName ZAI Agent" "${plistPath}"`)
    console.log("[patch-electron-dev] Updated Info.plist: name -> ZAI Agent")
  } catch (e) {
